...
def noodvensterkamer1():
    ...
    noodvenstercamerabeeldentabel = tk.Label(master=noodmenuframe1, background="royal blue",text="Camerabeelden: ")
    ...
    noodvenstercontactgegevenslabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Noodcontactgegevens", font=("",15))
    noodvenstercontactgegevenslabel.grid(row=1, column=0, padx=5, pady=5)

    noodvenstervoornaamlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Voornaam: \t\t"+ databasereader("""SELECT voornaam from persoon where noodpersoonid = 1"""))
    noodvenstervoornaamlabel.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstertussenvoegsellabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Tussenvoegsel: \t\t" + str(databasereader("""SELECT tussenvoegsel from persoon where noodpersoonid = 1""")))
    noodvenstertussenvoegsellabel.grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterachternaamlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Achternaam: \t\t"+ databasereader("""SELECT achternaam from persoon where noodpersoonid = 1"""))
    noodvensterachternaamlabel.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstergeslachtlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Geslacht: \t\t"+ databasereader("""SELECT geslacht from persoon where noodpersoonid = 1"""))
    noodvenstergeslachtlabel.grid(row=5, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstergeboortedatumlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Geboortedatum: \t\t"+ str(databasereader("""SELECT geboortedatum from persoon where noodpersoonid = 1""")))
    noodvenstergeboortedatumlabel.grid(row=6, column=0, padx=5, pady=5 ,sticky=tk.W)

    noodvenstertelefoonnummerlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Telefoonnummer: \t"+ databasereader("""SELECT telefoonnummer from persoon where noodpersoonid = 1"""))
    noodvenstertelefoonnummerlabel.grid(row=7, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterplaatsnaamlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Plaatsnaam: \t\t" + databasereader("""SELECT plaatsnaam from persoon where noodpersoonid = 1"""))
    noodvensterplaatsnaamlabel.grid(row=8, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterhuisnummerlabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Huisnummer: \t\t" + databasereader("""SELECT huisnummer from persoon where noodpersoonid = 1"""))
    noodvensterhuisnummerlabel.grid(row=9, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterpostcodelabel = tk.Label(master=noodmenuframe1, background="royal blue", text="Postcode: \t\t" + databasereader("""SELECT postcode from persoon where noodpersoonid = 1"""))
...
def noodvensterkamer2():
    ...
    noodvenstercamerabeeldentabel = tk.Label(master=noodmenuframe2, background="royal blue", text="Camerabeelden: ")
    noodvenstercamerabeeldentabel.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
    ...
    noodvenstercontactgegevenslabel = tk.Label(master=noodmenuframe2, background="royal blue",
                                               text="Noodcontactgegevens", font=("", 15))
    noodvenstercontactgegevenslabel.grid(row=1, column=0, padx=5, pady=5)

    noodvenstervoornaamlabel = tk.Label(master=noodmenuframe2, background="royal blue", text="Voornaam: \t\t" + databasereader("""SELECT voornaam from persoon where noodpersoonid = 2"""))
    noodvenstervoornaamlabel.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstertussenvoegsellabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Tussenvoegsel: \t\t" + str(databasereader("""SELECT tussenvoegsel from persoon where noodpersoonid = 2""")))
    noodvenstertussenvoegsellabel.grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterachternaamlabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Achternaam: \t\t" + databasereader("""SELECT achternaam from persoon where noodpersoonid = 2"""))
    noodvensterachternaamlabel.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstergeslachtlabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Geslacht: \t\t" + databasereader("""SELECT geslacht from persoon where noodpersoonid = 2"""))
    noodvenstergeslachtlabel.grid(row=5, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstergeboortedatumlabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Geboortedatum: \t\t" + str(databasereader("""SELECT geboortedatum from persoon where noodpersoonid = 2""")))
    noodvenstergeboortedatumlabel.grid(row=6, column=0, padx=5, pady=5, sticky=tk.W)

    noodvenstertelefoonnummerlabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Telefoonnummer: \t" + databasereader("""SELECT telefoonnummer from persoon where noodpersoonid = 2"""))
    noodvenstertelefoonnummerlabel.grid(row=7, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterplaatsnaamlabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Plaatsnaam: \t\t" + databasereader("""SELECT plaatsnaam from persoon where noodpersoonid = 2"""))
    noodvensterplaatsnaamlabel.grid(row=8, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterhuisnummerlabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Huisnummer: \t\t" + databasereader("""SELECT huisnummer from persoon where noodpersoonid = 2"""))
    noodvensterhuisnummerlabel.grid(row=9, column=0, padx=5, pady=5, sticky=tk.W)

    noodvensterpostcodelabel = tk.Label(master=noodmenuframe2, background="royal blue",text="Postcode: \t\t" + databasereader("""SELECT postcode from persoon where noodpersoonid = 2"""))
    noodvensterpostcodelabel.grid(row=10, column=0, padx=5, pady=5, sticky=tk.W)
...
def frames():
    ...
    noodmenuframe1.configure(background="royal blue")
    noodmenuframe1.pack()

    noodmenuframe2 = tk.Frame(root)
    noodmenuframe2.configure(background="royal blue")
    noodmenuframe2.pack()
